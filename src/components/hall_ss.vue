...
export default {
    ...
    data() {
        return {
            ...
            thing_ss: this.$route.params.val[7],
        ...
        showStu1() {
            if (this.desc == 'desc') {
                axios.get('http://localhost:3000/api/Stu/showStu',).then((response) => {
                    ...
                    for (let a = 0; a < this.dat.length; a++) {
                        if (this.all_form_id.includes(this.dat[a].form_id) && this.dat[a].form1.includes(this.thing_ss) && this.dat[a].time > this.time_all[0] && this.dat[a].time < this.time_all[1] ) {
