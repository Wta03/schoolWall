...
export default {
    ...
    methods: {
        ...
        add() {
            axios.post('http://localhost:3000/api/Stu/addStu1', { user_name: this.user_name, time: this.getFormatDate(), form1: this.thing, form2: this.something, img_src: '', have_name: this.who, form_id: 4, tx_imge: this.user1.circleUrl, name: this.user1.name }, {}).then((response) => { console.log(response); });
        },
        button_click() {
        ...
        tj() {
            if (this.thing != '问题' && this.thing != '' && this.something != '额外信息' && this.something != '') {
                this.open2();
                this.add();
